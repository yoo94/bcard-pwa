...
app.post('/server/onCreate', async (req, res) => {
  const { name, hpNum, company, email, image, auth } = req.body;

  try {
    const collection = await connectToDatabase();
    const result = await collection.insertOne({ name, hpNum, company, email, image, auth });
...
app.post('/server/selectData', async (req, res) => {
  const { auth } = req.body;
...
app.get('/server/selectData', async (req, res) => {
